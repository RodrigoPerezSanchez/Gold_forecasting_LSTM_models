# -*- coding: utf-8 -*-
"""
Created on Mon Apr 24 12:49:10 2023
...
dataset = pd.read_csv("../Capitulo_1/indicadores_cap_1_v3_DosMes.csv")
...
n_train_time = int(reframed.shape[0]*0.80) # 80% para entrenamiento #!!!!
...
model = Sequential()
model.add(LSTM(50, activation='relu', return_sequences=True, input_shape=(train_X.shape[1], train_X.shape[2]))) #!!! cambiar tipo de red LSTM y unidades LSTM: 50 (32-128)
#!!!! Activation - ('relu', 'tanh', 'sigmoid')
model.add(LSTM(50, activation='relu')) #activation relu o tanh
model.add(Dense(1))
...
ggplot(aes(x="minutos_test", y="valores"),df_final) + geom_line(aes(color = "Tipo"), size = 1) +\
       xlab("Minutos Test") + ylab("Precio Oro") + ggtitle("Stacked LSTM - Gold")



#%%
###########################################################################################
##########  CONSTRUCCIÓN DEL CSV - STACKED LSTM - CON HIPERPARÁMETROS #####################
###########################################################################################

"""
esto es un estudio de hyperparámetros, se dejará procesando el código durante el tiempo necesario para implementar todas las combinaciones posibles de hiper-parámetros.
Se repetirá el proceso tres veces para obtener una media de resultados (evitar resultados erroneos procedentes de la ejecución del modelo para una combinación específica)
"""
...
for combo in combinations:
        ...
        model = Sequential()
        model.add(LSTM(50, activation=combo[4], return_sequences=True, input_shape=(train_X.shape[1], train_X.shape[2]))) #!!!!
...
# Construir el DataFrame final para Stacked LSTM
df1 = pd.DataFrame(results)     

df1.to_csv("optimization_stacked_lstm.csv")
