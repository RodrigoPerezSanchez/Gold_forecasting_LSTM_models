# -*- coding: utf-8 -*-
"""
Created on Tue May 16 10:16:35 2023
...
n_train_time = int(reframed.shape[0]*0.8) # 80% para entrenamiento #!!!!
...
""" DISEÑO DEL MODELO -> BIDIRECTIONAL LSTM """

# difinimos el modelo

model = Sequential()
model.add(Bidirectional(LSTM(50, activation='tanh'), input_shape=(train_X.shape[1], train_X.shape[2])))
model.add(Dense(1))
...
ggplot(aes(x="minutos_test", y="valores"),df_final) + geom_line(aes(color = "Tipo"), size = 1) +\
       xlab("Minutos Test") + ylab("Precio Oro") + ggtitle("Bidirectional LSTM - Gold")


#%%

###########################################################################################
##########  CONSTRUCCIÓN DEL CSV - BIDIRECTIONAL LSTM - CON HIPERPARÁMETROS ###############
###########################################################################################
...
for combo in combinations:
        ...
        model = Sequential()
        model.add(Bidirectional(LSTM(50, activation=combo[4]), input_shape=(train_X.shape[1], train_X.shape[2]))) #!!!!
        model.add(Dense(1))
...
df2.to_csv("optimization_bidirectional_lstm.csv")
